...
As time goes on, how does Pakistans agri productivity improve relative to others? We see that the gap between Pakistan and India has widened considerably. Labor productivity growth is the slowest in South Asia between 2000 and 2019, but levels are still higher.
...
Replicating this analysis for every year we have data for, and controlling for Log GDP per capita, Log Agri Labor Productivity, and the agri share of GDP, Pakistans log Agri labor productivity is slower than expected for most years since 2000. Below we plot the residuals.
...
regress_dat %>% 
    ...
    theme_few() +
    labs(
         title = "Did Pakistans Agri Productivity Grow faster or Slower than Expected?",
...
Economic yield is the gross production value / area 
Relative economic yield how Pakistans economic yield relates to the average economic yield for that crop. 

Market proportion is the gross production value of pakistan relative to the total production value in the world. 



```{r fig.height = 10, fig.width = 10}
# want to know pakistans yield relative to global yield, and its ranking 
# want to compute econ_yield as well which is value over land area used

# names(global_agri_wide)

production_yearly <- global_agri_wide %>%
    group_by(Year, Item) %>%
    summarise(
              avg_econ_yield = mean(econ_yield, na.rm = T),
              avg_yield = mean(Yield, na.rm = T),
              total_area_harvested = sum(`Area harvested`, na.rm = T),
              total_production = sum(Production, na.rm = T),
              total_production_value = sum(`Gross Production Value`, na.rm = T)
    ) %>%
    ungroup()

pakistan_relative_productions <- global_agri_wide %>%
    filter(Area == "Pakistan") %>%
    filter(Year == base_year) %>%
    left_join(production_yearly %>% filter(Year == base_year) %>% select(-Year), by="Item") %>%
    mutate(
           relative_econ_yield = econ_yield / avg_econ_yield, 
           relative_yield = Yield / avg_yield, 
           production_proportion = Production / total_production, 
           market_proportion = `Gross Production Value` / total_production_value
    )

pakistan_relative_productions %>%
    arrange(-relative_econ_yield, -market_proportion) %>%
    select(Item, "Econ Yield"=econ_yield, "Relative Econ Yield"=relative_econ_yield, "Market Proportion"=market_proportion, "Production Proportion"=production_proportion) %>%
    slice_head(n = 50) %>%
    kable()

pakistan_relative_productions %>%
    ggplot(aes(x=market_proportion, y=relative_yield, label=Item)) +
    geom_text() +
    theme_few() + 
    labs(
         title = "Relative Yield vs Market Proportion",
         x = "Market Proportion",
         y = "Relative Yield"
    )

pakistan_relative_productions %>%
    filter((relative_yield < 2) & (market_proportion < 0.1)) %>%
    ggplot(aes(x=market_proportion, y=relative_yield, label=Item)) +
    geom_text() +
...
pakistan_relative_productions %>%
    filter(market_proportion < 0.1) %>%
    ggplot(aes(x=market_proportion, y=relative_econ_yield, label=Item)) +
    geom_text() +
    geom_hline(yintercept = 1, linetype="dotted") +
    theme_few() + 
    labs(
         title = "Relative Econ Yield vs Market Proportion",
         x = "Market Proportion",
         y = "Relative Econ Yield"
    )
...
pakistan_relative_productions %>%
    ggplot(aes(x=relative_yield, y=relative_econ_yield, size=`Area harvested`, label=Item)) +
    geom_point() +
    geom_hline(yintercept = 1, linetype="dotted") +
    ...
    theme_few() + 
    labs(
         title = "Relative Economic Yield vs Relative Yield",
         y = "Relative Economic Yield",
         x = "Relative Yield"
    )

pakistan_relative_productions %>%
    ggplot(aes(x=relative_yield, y=relative_econ_yield, label=Item)) +
    geom_text() +
    geom_hline(yintercept = 1, linetype="dotted") +
    geom_vline(xintercept = 1, linetype="dotted") +
    geom_abline(slope = 1, intercept = 0) +
...
pakistan_relative_productions %>%
    ggplot(aes(x=production_proportion, y=market_proportion, label=Item)) +
    geom_text() +
    geom_abline(slope = 1, intercept = 0) +
    theme_few() + 
    labs(
         title = "Market Proportion vs Production Proportion",
         y = "Market Proportion (pak $ / world $)",
         x = "Production Proportion (pak kg/world kg)"
    )
...
for(i in 1:nrow(items)) {
    ...
    yield_position        <- (yield_ranking %>% filter(Area == "Pakistan") %>% select(position))[[1]]
    pak_val               <- (yield_ranking %>% filter(Area == "Pakistan") %>% select(Value))[[1]]
    volume_position       <- (volume_ranking %>% filter(Area == "Pakistan") %>% select(position))[[1]]
    value_position        <- (value_ranking %>% filter(Area == "Pakistan") %>% select(position))[[1]]
    ...
    rankings <- rankings %>%
        add_row(
                item        = item,
                mean        = round(mean(yield_ranking$Value), 0),
                std_err     = round(sd(yield_ranking$Value), 0),
                max         = max(yield_ranking$Value),
                rating      = yield_position,
                vol_rating  = volume_position,
                pak_yield   = rel_row$Yield[[1]],
                rel_yield   = pak_val / mean(yield_ranking$Value, na.rm = T),
...
What does Pakistan produce? I took 2019 because it is recent and pre-covid.
```{r fig.width = 10, fig.height = 10}
...
The new approach is to look at things we produce a lot of, and examine the difference between crops grown well and crops grown poorly. 
